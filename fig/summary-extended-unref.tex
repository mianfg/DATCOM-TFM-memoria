...
\end{minipage}};
% Text Node
\draw (623.76,195.55) node  [color={rgb, 255:red, 0; green, 0; blue, 0 }  ,opacity=1 ] [align=left] {\begin{minipage}[lt]{220.92pt}\setlength\topsep{0pt}
-- sound {\small{($\varphi\text{ theorem}\Rightarrow\varphi\text{ true}$)}}\\
-- decidable {\small{($\varphi\text{ true}\Rightarrow\varphi\text{ theorem}$)}}\\
-- (semantically) complete {\small{($\varphi\text{ or }\neg\varphi\text{ are theorems}$)}}\\
-- syntactically complete {\small{($\varphi\text{ and }\neg\varphi\text{ are not theorems}$)}}\\
-- consistent {\small{($\varphi\text{ and }\neg\varphi\text{ are not true}$)}}
\end{minipage}};
% Text Node
