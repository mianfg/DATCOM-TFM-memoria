...
The first goal of this work is to automatically generate rules that describe plant conditions, utilizing functional descriptions of sensors, Piping and Instrumentation Diagrams (P\&IDs), and other internal documentation. This involves leveraging Large Language Models (LLMs) to interpret complex engineering information and translate it into actionable rules for condition monitoring, anomaly detection, or predictive maintenance.
...
Finally, confidence elicitation can be used by prompting LLMs to explicitly state their confidence level alongside their answers. Frameworks like SteerConf \cite{zhou_steerconf_2025} calibrate the model's responses by ``steering'' the model's confidence scores (e.g., to be ``very cautious'' or ``confident''). This helps calibrate the reported confidence. Consistency in confidence scores across different steering levels indicates higher certainty.
...
Survival analysis is a statistical method for analyzing time-to-event data, commonly used in reliability engineering and prognostics. The goal is to predict when an event (such as equipment failure) will occur based on historical data and current system conditions.

\subsection{Traditional approaches}

Classical survival analysis methods include Cox proportional hazards models, Kaplan-Meier estimators, and parametric survival models. However, these traditional approaches often struggle with high-dimensional data and complex non-linear relationships.

\subsection{Deep learning for survival analysis}

Recent advances in deep learning have shown promise in improving survival predictions. Deep learning models can handle high-dimensional inputs and capture complex patterns in the data \cite{wiegrebe_deep_2024}. These methods have been applied to remaining useful life (RUL) estimation in prognostics \cite{zhang_multiobjective_2017}, where the goal is to predict how much longer a system will operate before failure.

\subsection{Challenges}

Key challenges in survival prediction include handling censored data (where the event has not yet occurred for some observations), dealing with imbalanced datasets, and ensuring model interpretability in safety-critical applications.
